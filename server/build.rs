...
const WASM_REPO: &str = "../gridpaint2";

fn main() {
    println!("cargo:rerun-if-changed=../gridpaint2/src");
    println!("cargo:rerun-if-changed=../gridpaint2/Cargo.toml");
    ...
    if is_release {
        wasm_bindgen.arg(format!(
            "{out_dir_str}/wasm32-unknown-unknown/release/gridpaint2.wasm",
        ));
    } else {
        wasm_bindgen.arg(format!(
            "{out_dir_str}/wasm32-unknown-unknown/debug/gridpaint2.wasm",
        ));
    }
